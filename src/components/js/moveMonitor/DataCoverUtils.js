...
import echartsYunNanMapOption from '../../echart/EchartsYunNanMapOption.js';
...
export default {
	...
	dataInit: function (data, option) {
		...
		for (let key in data) {
			...
			if (key === 'x') {
				...
				for (let childKey in oneData) {
					...
					if (keyValue.indexOf("指数") < 0 && keyValue.indexOf("%") < 0) {
						init.yAxisIndex = 0;
						init.type = 'bar';
					...
					if (init.setflag == 1) {
						init.name = this.areaMap[childKey] + ":" + init.targetName;
						if (childKey == 'quan_guo') {
							init.color = '#FFA516';
						}
						if (childKey == 'yun_nan') {
		...
		if (JSON.stringify(yData.yAxis[0]) === '{}') {
			yData.yAxis.shift();
			yData.yAxis[0].axisLabel = {
				formatter: '{value} %'
			}
			// yData.y
			for (let i in yData.y) {
				yData.y[i].yAxisIndex = 0;

			}
		}
		if (yData.x.length > 12) {
			yData.x[yData.x.length - 3] = yData.x[yData.x.length - 3] + '(预测值)';
			yData.x[yData.x.length - 2] = yData.x[yData.x.length - 2] + '(预测值)';
			yData.x[yData.x.length - 1] = yData.x[yData.x.length - 1] + '(预测值)';
		}
		return yData;
	...
	dataNationInit: function (data, option) {
		...
		for (let key in data) {
			...
			if (key === 'x') {
				...
				if (option[key].indexOf("%") < 0 && option[key].indexOf("指数") < 0) {
					init.yAxisIndex = 0;
					init.type = 'bar';
					init.color = '#03a9f4';
					left = true;
	 ...
	 * 指标名称
	 */
	dataNationOneInit: function (data, option, option2) {
		const yData = {
			y: [],
			x: [],
			yAxis: [{}, {}],
		...
		let minRight = 100000000;
		let maxRight = -100000000;
		let color = ['#FF7F50', '#03a9f4', '#da70d6', '#67abbd', '#0265ba']
		for (let key in data) {
			const oneData = data[key];
			//如果是x，则表示横坐标单位，将key和value用-赋值添加到x中，例如'650000-新疆维吾尔自治区'
			if (key === 'x') {
				...
				if (keyValue.indexOf("%") < 0 && keyValue.indexOf("指数") < 0) {
					init.yAxisIndex = 0;
					init.type = 'bar';
					// init.color = '#03a9f4';
					// init.color = color[key];
	 ...
	 * 较为简单，直接赋值即可
	 */
	dataAreaInit: function (data, keyValue) {
		const yData = {
			y: [],
			x: [],
			yAxis: [],
	 ...
	 * 数据的单位一般为%和亿元，也有为空字符串的情况
	 */
	dataAreaMapInit2: function (data, suffix) {
		const option = {
			...echartsYunNanMapOption
		}
		option.series[0].data = [];
		...
		for (let key in data) {
			if (key === 'x') {
				...
				for (let xKey in data[key]) {
					const ob = data[key][xKey];
					xData.push(ob.name + '-' + ob.id);
				}
				yData.x = xData;
			} else {
				const init = {
		...
		for (let key in data) {
			if (key !== "code") {
				if (key === 'x') {
					const xData = [];
					for (let xKey in data[key]) {
						xData.push(data[key][xKey] + '-' + data.code[xKey]);
					}
					yData.x = xData;
				} else {
					const init = {
