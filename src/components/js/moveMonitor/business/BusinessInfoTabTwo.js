...
root.methods = {
	...
	getAreaInfo: async function () {
		const echartsData = DataCoverUtils.dataAreaMapInit(qiye.qiyeData, '个');
		this.echartsData.echartsYunNanMapData = echartsData;
		// this.slotsData = {
		// 	data: qiye.qiyeData[0]
		// }
		...
		const res = await this.componentsMoudel.startPost("distribution/getIndustrialDistributionDataByParam", data, false);
	...
	//插件数据构建
	scopedSlotsParameter: async function (areaCode) {
		...
		const res = await this.componentsMoudel.startPost("distribution/getIndustrialDistributionDataByParam", data2, false);
		this.slotsData = {
			data: res.body.data,
			areaName: '昆明市'
		}
	...
	areaEnClick: async function (chooseArea) {
		console.log(chooseArea.data.name);
		this.setAreaName = chooseArea.data.name;
		let data = {
			index: this.id,
			areaCode: chooseArea.data.cityCode,
			"curPage": 1,
			"limit": 1000000
		};
		const res = await this.componentsMoudel.startPost("distribution/getIndustrialDistributionDataByParam", data, false);
		...
		this.slotsData = {
			data: res.body.data,
			areaName: chooseArea.data.name
		}
		// distribution/getIndustrialDistributionDataByParam
		...
		// 			this.thisChooseArea = { ...this.titleName[1] };
